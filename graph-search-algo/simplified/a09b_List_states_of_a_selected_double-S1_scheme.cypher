//a09b List states of a selected double-S1 scheme
...
match p = (s2)<-[r12prim:DECAY]-(s1prim)-[r10prim:DECAY]->(s0)<-[r10:DECAY]-(s1)-[r12:DECAY]->(s2) where r12.energy_diff > min_laser_nm and r12.energy_diff < max_laser_nm and r12.branching_ratio >= minimal_considered_BR_per_scheme and r12prim.energy_diff > min_laser_nm and r12prim.energy_diff < max_laser_nm and r12prim.branching_ratio >= minimal_considered_BRprim_per_scheme and s2.J < max_J

with apoc.coll.sort(apoc.coll.toSet(apoc.coll.flatten(collect(DISTINCT s0.id) + collect(DISTINCT s1.id) + collect(DISTINCT s1prim.id) + collect(DISTINCT s2.id)))) as all_ids
