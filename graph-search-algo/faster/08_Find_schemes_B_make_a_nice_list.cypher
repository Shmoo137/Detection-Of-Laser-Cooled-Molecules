...
match (s0:EnergyWiseStartingState)<-[r1:DECAY]-(s1:ReachableExcitedState) where s0.energy < initial_energy and s0.tau > min_starting_tau and r1.energy_diff > min_laser_nm and r1.energy_diff < max_laser_nm and s0.J < max_J and s1.J < max_J
with s1, apoc.coll.max([4, s0.energy*0.62485285866738]) as initial_temp, s0.id as starting_id, num_decays as num_decays, min_laser_nm as min_laser_nm, max_laser_nm as max_laser_nm, br_cutoff as br_cutoff, max_J as max_J, min_starting_tau as min_starting_tau
...
with s1, collect(r2.branching_ratio) as br_list, num_decays as num_decays, initial_temp as initial_temp, starting_id as starting_id, min_laser_nm as min_laser_nm, max_laser_nm as max_laser_nm, br_cutoff as br_cutoff, max_J as max_J, min_starting_tau as min_starting_tau
with s1, apoc.coll.min(apoc.coll.reverse(apoc.coll.sort(br_list))[0..num_decays]) as minimal_considered_BR_per_scheme, initial_temp as initial_temp, starting_id as starting_id, min_laser_nm as min_laser_nm, max_laser_nm as max_laser_nm, br_cutoff as br_cutoff, max_J as max_J, min_starting_tau as min_starting_tau

match (s1:ReachableExcitedState)-[r2:DECAY]->(s2:ReachableState) where r2.energy_diff > min_laser_nm and r2.energy_diff < max_laser_nm and s2.J < max_J and s2.tau > min_starting_tau and r2.branching_ratio > br_cutoff and r2.branching_ratio >= minimal_considered_BR_per_scheme
